	...
	var methods = {
		...
		getAuthToken: {
			...
			callback:		function(params, req, res, callback) {
				...
				if (!params.systoken || params.systoken != Gamify.settings.systoken) {
					// No system token, let's make sure we have enough info to return an authtoken
					if (!params.user.email || !params.user.password) {
						callback(scope.Gamify.api.errorResponse('You need to provide at least an email and a password.'));
					}
				}
				scope.mongo.find({
					collection:	'users',
				...
				/*scope.mongo.find({
					...
					if (response.length == 0) {
						...
						// Check if there is already a valid authToken:
						scope.mongo.find({
							collection:	'authtokens',
							query:		{
								uid:		response[0].uid,
							...
							//Gamify.log("response2", response2);
							if (response2.length == 0) {
								//Gamify.log("Creating new token", "");
								...
								// Return the current authToken, update the validity
								callback({
									valid: 		true,
									authtoken:	response2[0].token,
									user:		response[0].uid
								});
			...
			callback:		function(params, req, res, callback) {
				...
				var create_account = function() {
					...
					scope.mongo.insert({
						...
						scope.Gamify.api.execute("user","getAuthToken", {user: {uid:uid},systoken:Gamify.settings.systoken}, function(response) {
							callback(response);
							/*
								...
								if (user) {
									...
									if (auth_response) {
										scope.Gamify.api.execute("user","set", {
											...
											scope.Gamify.api.execute("user","getAuthToken", {user: {fbuid:data.id*1},systoken:Gamify.settings.systoken}, function(response2) {
												callback(response2);
											});
										});
									} else {
										// User is logging in using facebook.
										scope.Gamify.api.execute("user","getAuthToken", {user: {fbuid:data.id*1},systoken:Gamify.settings.systoken}, function(response) {
											callback(response);
		...
		fblogin: {
			...
			callback:		function(params, req, res, callback) {
				...
				// Check if there is a FB account link to either the fb uid or email
				var _fbaccount = function(data, cb) {
					scope.mongo.find({
						collection:	'users',
						query:		{
							$or: [{
				...
				// Check if there is an account linked to the authtoken
				var _authaccount = function(cb) {
					scope.Gamify.api.execute("user","validateAuthToken", {authtoken: params.authtoken}, function(response) {
						if (response && response.valid) {
							cb(response);
						} else {
				...
				var _listFriends = function(data, cb) {
					fbclient.me.friends(function(err, friendlist) {
						var friends = [];
						var i;
						var l = friendlist.length;
				...
				var _create	= function(data, cb) {
					...
					// List the friends
					_listFriends(data, function(friends) {
				...
				*/
				...
				// Get the fb data using the fbuid and fbauthtoken
				var fbclient = fbapi.user(params.fbtoken);
				fbclient.me.info(function(err, data) {
					//Gamify.log("data", data);
					if (err) {
						...
						var fbaccount 	= false;	// false by default
						var authaccount = false;	// false by default
						
						var callstack = new Gamify.stack();
						
						// get the fbaccount if there is one
						callstack.add(function(p, cb) {
							_fbaccount(data, function(response) {
								fbaccount = response;
								cb();
							});
						}, {});
						
						// Get the authaccount if there is one
						if (params.authtoken) {
							callstack.add(function(p, cb) {
								_authaccount(function(response) {
									authaccount = response;
									cb();
								});
							}, {});
						}
						
						// Now that we have both requests int he stack, execute
						callstack.process(function() {
							...
							if (authaccount && fbaccount) {
								...
								if (authaccount.user && authaccount.user.uid == fbaccount.uid) {
									// Same account
									// Update the list of friends (background)
									_listFriends(data, function(friends) {
									...
									// Different accounts.
									// We can't link that facebook account, it already exist!
									callback(Gamify.api.errorResponse("There is already an account linked to that facebook profile. Please logout and re-login using facebook."));
								}
								...
								// Only one account
								if (authaccount) {
									// No fbaccount, but session (authtoken) found
									//Gamify.log("Required", 	"No implementation: authaccount");
								} else if (fbaccount) {
									
									// No session, but a fb account found
									// Update the friend, override the fbuid for that account (in case it was linked to another facebook account)
									_listFriends(data, function(friends) {
									...
									_create(data, function(response) {
										//Gamify.log("Account created: ", response);
										// Returns a session.
										callback(response);
									});
								}
		...
		detachFacebook: {
			...
			params:			{},
			auth:			"authtoken",
			description:	"Detach the facebook account from a fleetwit account",
			status:			'stable',
			version:		1.0,
			callback:		function(params, req, res, callback) {
